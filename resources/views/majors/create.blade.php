        ...
        <form action="{{ route('majors.store') }}" method="POST"
            ...
            <div class="grid sm:grid-cols-2 gap-4">
                ...
                <div class="flex flex-col gap-2">
                    ...
                    <input type="text" id="code" name="code"
                        class="px-3 py-2 border border-zinc-300 bg-slate-50"
                        placeholder="Major Code (e.g., CS101)" value="{{ old('code') }}">
                    @error('code')
                        <div class="text-red-500">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="flex flex-col gap-2">
                ...
                <textarea id="description" name="description"
                    class="px-3 py-2 border border-zinc-300 bg-slate-50" placeholder="Description of the major">{{ old('description') }}</textarea>
                @error('description')
                    <div class="text-red-500">{{ $message }}</div>
                @enderror
            </div>
